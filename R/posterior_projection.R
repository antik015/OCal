...
function(y, x, phi_sq, psi, nu, nmcmc, burnin, sig = sigT, m, prior = "BART", proj_type = "infinite", computer_model_functions, nugget = 0, alpha = 0.95, MH_sd = 0.1, prior_sd)
{
  t1 = Sys.time()
  n = length(x)
  ...
  if(prior == "GP")
  {
    ...
    K_all = cov_mat(c(x, x_t), phi_sq, psi, nu) + nugget*diag(n+m)
    K = K_all[1:n, 1:n]
