...
function(Y, x, phi_sq, nu, psi, nmcmc, burnin, Sig = SigT, m, prior = "BART", computer_model_functions, nugget = 0,  MH_sd = 0.1, prior_sd)
{
  t1 = Sys.time()
  n = length(x)
  ...
  if(prior == "GP")
  {
    ...
    cov_mat = function(x, phi_sq, nu, psi)
    {
      n = length(x)
      K = matrix(0, n, n)
      for(i in 1:(n-1))
      {
        for(j in (i+1):n)
        {
          K[i,j] = K[j,i] = calib_cov(x[i], x[j], phi_sq, nu, psi)
        }
      }
      diag(K) = phi_sq
      return(K)
    }
    K_all = cov_mat(c(x, x_t), phi_sq, nu, psi) + nugget*diag(n+m)
    K = K_all[1:n, 1:n]
